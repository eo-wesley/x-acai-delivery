...
function RatingForm({ orderId, customerName }: { orderId: string; customerName?: string }) {
    ...
    useEffect(() => {
        // Check if already rated (using the track endpoint which now includes rating)
        fetch(`${API}/api/track/${orderId}`).then(r => r.json()).then(data => {
    ...
    const submit = async () => {
        ...
        const res = await fetch(`${API}/api/orders/${orderId}/rate`, {
            ...
            body: JSON.stringify({ rating: stars, feedback: comment }),
        });
...
function GPSSimulator({ status }: { status: string }) {
    const isDelivering = status === 'delivering' || status === 'out_for_delivery';
    ...
    return (
        <div className="bg-gray-100 rounded-3xl overflow-hidden h-48 relative shadow-inner border border-gray-200/50">
            ...
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 200">
                ...
                {/* Restaurant Icon */}
                <circle cx="50" cy="150" r="8" fill="#1E293B" />
                <text x="42" y="175" className="text-[10px] font-black fill-gray-400">LOJA</text>

                {/* Customer Icon */}
                <circle cx="350" cy="50" r="8" fill="#10B981" />
                <text x="325" y="75" className="text-[10px] font-black fill-gray-400">VOCÊ</text>

                {/* Driver Marker */}
                <g className="transition-all duration-300 ease-linear" style={{ transform: `translate(${(progress * 3)}px, ${-progress * 0.5}px)` }}>
                    <circle cx="50" cy="150" r="14" fill="#8B5CF6" className="animate-pulse shadow-xl" />
                    <text x="44" y="155" className="text-[12px]">🏍️</text>
                </g>
            </svg>

            <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md px-4 py-2 rounded-2xl flex items-center justify-between shadow-lg border border-white">
                <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-purple-500 rounded-full animate-ping" />
                    <span className="text-[10px] font-black text-purple-900 uppercase">Localização em Tempo Real</span>
                </div>
                <span className="text-[10px] font-black text-gray-400 uppercase">Simulação Ativa</span>
            </div>
        </div>
    );
}
...
export default function OrderStatusPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="min-h-screen bg-[#FDFCFE] font-sans">
            <div className="max-w-md mx-auto bg-white shadow-sm min-h-screen pb-32 relative overflow-x-hidden">
                {/* Status change flash */}
                {justChanged && (
                ...
                <div className="px-6 -mt-8 space-y-6 relative z-20">
                    ...
                    {/* Chat with Support Button - PREMIUM ADDITION */}
                    <a href={supportLink} target="_blank" rel="noreferrer" className="block transform transition hover:scale-[1.02] active:scale-[0.98]">
                        <div className="bg-green-500 rounded-3xl p-5 flex items-center justify-between text-white shadow-xl shadow-green-200">
                            <div className="flex items-center gap-4">
                                <div className="bg-white/20 p-3 rounded-2xl backdrop-blur-sm">
                                    <span className="text-2xl">💬</span>
                                </div>
                                <div>
                                    <p className="font-black text-sm uppercase tracking-tight">Precisa de ajuda?</p>
                                    <p className="text-[10px] font-medium opacity-80 uppercase tracking-widest">Fale conosco no WhatsApp</p>
                                </div>
                            </div>
                            <div className="bg-white text-green-600 w-8 h-8 rounded-full flex items-center justify-center font-black">→</div>
                        </div>
                    </a>
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-xl shadow-purple-900/5 border border-purple-50/50 overflow-hidden relative">
                        ...
                        <div className="flex items-center gap-4">
                            {order.restaurant_logo ? (
                                <img src={order.restaurant_logo} alt="Logo" className="w-16 h-16 rounded-[20px] object-cover shadow-lg border border-gray-100" />
                            ) : (
                                <div className="w-16 h-16 bg-gradient-to-br from-purple-100 to-purple-200 text-purple-700 font-black flex items-center justify-center rounded-[20px] text-2xl shadow-inner border border-purple-50">
                                    {order.restaurant_name?.charAt(0) || 'X'}
                                </div>
                            )}
                            <div className="flex-1">
                                <p className="font-black text-gray-800 uppercase tracking-tight leading-tight text-lg">{order.restaurant_name || 'X-Açaí Delivery'}</p>
                                <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mt-1">📍 {order.restaurant_address || 'Unidade Principal'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-xl shadow-purple-900/5 border border-purple-50/50">
                        ...
                        {order.customer_name && <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mt-4">Destinatário: <span className="text-gray-800">{order.customer_name}</span></p>}
                    </div>

                    <div className="text-center py-12">
                        <p className="text-[9px] text-gray-300 font-black uppercase tracking-[0.4em]">X-Açaí Delivery Platform</p>
                ...
                <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white/90 backdrop-blur-2xl border-t border-purple-50 p-6 z-40 shadow-[0_-20px_50px_rgba(0,0,0,0.08)]">
                    <Link href="/">
                        <button className="w-full bg-gradient-to-r from-purple-700 to-purple-800 hover:from-purple-800 hover:to-purple-900 text-white font-black py-5 rounded-[28px] shadow-2xl shadow-purple-200 transition active:scale-[0.98] flex items-center justify-center gap-3 group overflow-hidden relative">
